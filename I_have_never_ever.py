def start(input, id, database):
    ...
    if not mode.startswith(game):
        ...
        text = 'Я никогда не...\n' + '\n'.join([ str(i + 1) + ') ' + j for i, j in enumerate(I_have_never_ever_biblio.questions)])
...
def return_start():
    text = 'Вы можете использовать команду "пропустить" с любым числом, чтобы перейти к более сложным ситуациям.\n\n' \
           'Если хотите - кто-то из команды разработчиков сыграет с вами.\n\n' \
           'Приятной игры!'
    speech = text
...
def return_another_rules():
    text = 'Заранее нужно приготовит фишки, это могут быть спички, монеты, крупная фасоль и так далее. ' \
